...
def positionCollision(local):
    colisao = []
    arq = open(local, 'r')
    for i in arq:
        i = i.split()
        colisao.append(pygame.Rect(int(i[0]), int(i[1]), 16, 16))
    arq.close()

    return colisao


def detectaColisao(before, collision, npc_list, door_list, cen, prot):
...
def movePlayer(screen, collision, npc_list, door_list, cen, prot):
    before = [prot.pos[0], prot.pos[1], cen.pos[0], cen.pos[1]]

    cen.moveCenario()
    prot.movePlayer(cen.limites)
    detectaColisao(before, collision, npc_list, door_list, cen, prot)

    screen.blit(pygame.image.load(cen.img), (cen.pos[0], cen.pos[1]))
...
def acao(npc_list, door_list, cen, prot):
    for npc in npc_list:
    ...
    for door in door_list:
        colide = door.area_interacao.move(cen.pos[0], cen.pos[1])
        if prot.rect.colliderect(colide):
            cen.setImg(door.cenario)


# Desenvolvedor
def visualizaColisao(screen, collision, door_list, npc_list, cen, prot):
